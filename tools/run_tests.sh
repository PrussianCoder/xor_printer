...
#   ./run_tests.sh ../sample_code/sample_code.py 0-9   # 範囲指定
...
if [ $# -eq 0 ]; then
    ...
    echo "  $0 ../sample_code/sample_code.py 0-9     # Range specification (0000-0009)"
    echo ""
...
case "$extension" in
    "py")
        execute_command="python $solver_file"
...
# genがコンパイルされていない場合はコンパイルする
if [ ! -f "gen" ]; then
    echo "gen not found. Compiling gen.cpp..."
    if ! g++ -std=c++23 -O2 gen.cpp -o gen; then
        echo "Error: Failed to compile gen.cpp"
        exit 1
    fi
    echo "Compiled gen successfully."
fi
...
# visがコンパイルされていない場合はコンパイルする
if [ ! -f "../tools/vis" ]; then
    echo "vis not found. Compiling vis.cpp..."
    if ! g++ -std=c++23 -O2 ../tools/vis.cpp -o ../tools/vis; then
        echo "Error: Failed to compile vis.cpp"
        exit 1
    fi
    echo "Compiled vis successfully."
fi
